...
// 사용자가 "4700"이라고 치면 시드 표기("$4,700")와 나란히 놓였을 때 티가 난다.
// 숫자만 뽑아 천단위 쉼표를 붙인다. 숫자가 없으면(예: "협의") 손대지 않는다.
function formatPrice(raw: string, suffix = "") {
  const digits = raw.replace(/[^\d]/g, "");
  if (!digits) return raw;
  return `$${Number(digits).toLocaleString("en-US")}${suffix}`;
}
...
export function userFleaToFleaItem(p: RawUserFlea): FleaItem {
  ...
  return {
    ...
    seller: "나",
    sellerEmoji: "👤",
    // 가입 연도를 받은 적이 없다. 등록 연도를 "활동 연도"로 쓰면 사실과 다르므로 비워 둔다
    sellerSince: "",
    sellerDeals: 0,
...
export function userRealtyToRealtyItem(p: RawUserRealty): RealtyItem {
  ...
  return {
    ...
    bg: meta.bg,
    title: p.title,
    price: formatPrice(p.price, isSale ? "" : "/월"),
    area: p.area,
...
export function userBizToBusiness(p: RawUserBiz): Business {
  ...
  return {
    ...
    reviewCount: 0,
    isOpen: true,
    // 선택 항목은 비면 빈 문자열 — 화면에서 "미입력"/"—" 대신 그 줄째로 숨긴다
