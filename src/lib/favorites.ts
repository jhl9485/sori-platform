...
export const ALL_FAV_ITEMS: FavItem[] = [
  ...
  // 라벨·아이콘·색을 여기에 다시 적으면 data/categories.ts와 두 벌이 되어 조용히 어긋난다.
  // (커뮤니티 탭에서 이름을 바꿔도 홈 즐겨찾기·하단바에는 반영이 안 되는 식)
  // → 카테고리 목록은 data/categories.ts 하나만 두고 여기서는 가져다 쓴다.
  //   id에 cat_ 접두사를 붙이는 이유: 메인 메뉴 id("jobs")와 카테고리 id("job")가 겹치기 때문.
